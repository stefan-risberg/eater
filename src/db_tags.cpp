...
bool DB_Tags::init()
{
    if (!DB::tableExists(db, tb_tags)) {
        std::vector<std::string> col_names = {c_id, c_tags_text};

        std::vector<std::string> col_types = {"integer primary key",
                                              "text not null"};
    ...
    if (!DB::tableExists(db, tb_food_tags)) {
        std::vector<std::string> col_names = {c_id, c_tags_id, c_foods_id};
